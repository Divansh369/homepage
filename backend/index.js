...
const HOST_TO_LISTEN = process.env.BACKEND_HOST || '0.0.0.0';
...
// --- Middleware Setup ---
const allowedOrigins = [ `http://localhost:1025`, `http://localhost:1026`, `http://100.94.150.11:1025`, `http://localhost`, `http://100.94.150.11`, 'http://vivo', 'http://vivo:1025' ];
app.use(cors({
    origin: (origin, cb) => {
...
api.get('/projects', async (req, res) => {
    try {
        ...
        res.json(projects.map(p => {
            let iconPath = null;
            if (p.icon_filename) {
                iconPath = p.icon_filename.startsWith('/uploads') ? 
                           `${PUBLIC_URL}${p.icon_filename}` : 
                           `${PUBLIC_URL}/project_icons/${encodeURIComponent(p.project_name)}/${encodeURIComponent(p.icon_filename)}`;
            }
            return {
                id: p.id, project_name: p.project_name, description: p.description,
                icon_filename: p.icon_filename, icon_path: iconPath,
                start_command: p.start_command, stop_command: p.stop_command,
                port: p.port, host: p.host, scheme: p.scheme,
                cards: p.Labels.map(l => ({ card_id: l.id, label_id: l.id, label_name: l.label_name })),
                github_url: p.github_url, deployed_url: p.deployed_url, notes: p.notes
            };
        }));
    } catch (e) { res.status(500).json({error: e.message}); }
});
...
api.post('/add-project', async (req, res) => {
    const { projectName, description, iconFilename, startCommand, stopCommand, scheme, port, host, github_url, deployed_url, notes, selectedLabels = [] } = req.body;
    const transaction = await sequelize.transaction();
    try {
        const highestOrder = (await Project.max('order', { transaction })) || 0;
        const [project, created] = await Project.findOrCreate({
            where: { project_name: projectName.trim() },
            defaults: { description, icon_filename: iconFilename, start_command: startCommand.trim(), stop_command: stopCommand?.trim(), scheme, port: parseInt(port), host: (host?.trim() || null), order: highestOrder + 1, github_url: github_url || null, deployed_url: deployed_url || null, notes: notes || null }, transaction
        });
...
api.put('/projects/:id', async (req, res) => {
    ...
    try {
        ...
        await project.update({
            project_name: projectName.trim(), description, icon_filename: iconFilename, start_command: startCommand.trim(),
            stop_command: stopCommand?.trim(), scheme, port: parseInt(port), host: (host?.trim() || null),
            github_url: github_url || null, deployed_url: deployed_url || null, notes: notes || null
        }, { transaction });
...
api.post('/stop-project', async (req, res) => {
    ...
    try {
        ...
        exec(project.stop_command, { cwd: path.join(PROJECTS_BASE_PATH, project.project_name) }, (error) => {
...
api.post('/add-label', async (req, res) => {
    ...
    try {
        const [, created] = await Label.findOrCreate({ where: { label_name: name.trim() }, defaults: { label_type: type } });
        if (!created) return res.status(409).json({ error: `Label '${name}' already exists.` });
        res.status(201).json({ message: `Label '${name}' added.` });
    } catch (error) { res.status(500).json({ error: error.message }); }
});
...
api.delete('/labels/:id', async (req, res) => {
    try {
        const label = await Label.findByPk(req.params.id);
        if (label) { await label.setProjects([]); await label.destroy(); }
...
async function start() {
    try {
        ...
        server.listen(port, HOST_TO_LISTEN, () => { console.log(`🚀 Server ready at http://${HOST_TO_LISTEN}:${port}`); });
